...
import { navigate } from "svelte-routing";
  ...
  fetch(API_ROOT_URL + `/games?` + new URLSearchParams({
      pageIndex: pageIndex.toString(),
      pageLength: pageLength.toString(),
  }), {
    headers: {...generateUserHeaders(OPENSKULL_USER_ID, OPENSKULL_USER_SECRET)}
  })
  .then(res => res.json())
...
<ul>
  {#each games as game (game.id)}
    <li><button on:click={() => navigate("/games/" + game.id)}>{game.playerCount} Game: {game.gameComplete ? "Completed" : "Ongoing"}</button></li>
  {/each}
</ul>
{/if}
{:else}
