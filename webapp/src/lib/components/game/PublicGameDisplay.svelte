<script lang="ts">
  ...
  const revealedCards = (fnGame: PublicGame, fnIndex: number) => fnGame.roundPlayerCardsRevealed[fnGame.roundNumber - 1][fnIndex]
  const revealedCardCount = (fnGame: PublicGame, fnIndex: number) => fnGame.roundPlayerCardsRevealed[fnGame.roundNumber - 1][fnIndex].length
  const unrevealedCardCount = (fnGame: PublicGame, fnIndex: number) => fnGame.roundCountPlayerCardsPlayed[fnGame.roundNumber - 1][fnIndex] - fnGame.roundPlayerCardsRevealed[fnGame.roundNumber - 1][fnIndex].length
  const unplayedCardCount = (fnGame: PublicGame, fnIndex: number) => fnGame.currentCountPlayerCardsAvailable[fnIndex] - fnGame.roundCountPlayerCardsPlayed[fnGame.roundNumber - 1][fnIndex]
  const lostCardCount = (fnGame: PublicGame, fnIndex: number) => fnGame.playerCardStartingCount - fnGame.currentCountPlayerCardsAvailable[fnIndex]
