...
public class GameController : ControllerBase
{
    ...
    [Route("games/join")]
    [HttpGet]
    public async Task<ActionResult<PlayerQueueStatus>> GetQueueStatus()
    {
        ...
        var playerQueuePosition = await _gameCreationQueue.FindPlayerInQueues(playerId);
        if (playerQueuePosition.IsFailure) {
            switch(playerQueuePosition.Error) {
                case PlayerQueueStatusError.PlayerNotInAnyQueues:
                    return NoContent();
                default:
                    throw new NotImplementedException();
            }
        }
        return playerQueuePosition.Value;
    }

    
    [Route("games/leave")]
    [HttpPost]
    public async Task<ActionResult> LeaveQueues()
    {
        StringValues rawPlayerId;
        Guid playerId;
        Request.Headers.TryGetValue("X-OpenSkull-UserId", out rawPlayerId);
        ...
        var res = await _gameCreationQueue.LeaveQueues(playerId);
        if (res.IsFailure) {
            switch (res.Error) {
                case PlayerQueueLeaveError.PlayerNotInAnyQueues:
                    return BadRequest("Player not in any queues");
                default:
                    throw new NotImplementedException();
            }
        }
        ...
        if (queueJoinResult.IsFailure) {
            switch(queueJoinResult.Error) {
                case QueueJoinError.OutsideGameSize:
                    return BadRequest("No game queues of that size available");
                case QueueJoinError.AlreadyInQueue:
                    return BadRequest("Already queueing for a game");
                default:
                    throw new NotImplementedException();
            }
        }
